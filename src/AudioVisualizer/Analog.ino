...
void drawNeedleLeft(boolean init) {
    ...
    if (rms1.available() || init) {
        ...
        // peak LED ON
        if ((20.0f * log10f(peak1.read() * 2.0f) + 3.0f) >= (float)ANALOG_PeakLEDdB) { peakLEDTrigger = true; peakLEDTime = 0; }
...
void drawNeedleRight(boolean init) {
    ...
    if (rms2.available() || init) {
        ...
        // peak LED ON
        if (20.0f * log10f(peak2.read() * 2.0f) + 3.0f >= (float)ANALOG_PeakLEDdB) { peakLEDTrigger = true; peakLEDTime = 0; }
