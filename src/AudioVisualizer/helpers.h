...
float rmsAmplitudeTodB(float amplitude, int16_t lr /*0=left, 1=right*/) {
    ...
    if (lr == 0) { dB += RMSCorrectionLeft / 100.0f; dBRMSL = dB; }  // left
    if (lr == 1) { dB += RMSCorrectionRight / 100.0f; dBRMSR = dB; } // right
...
float peakAmplitudeTodB(float amplitude, int16_t lr /*0=left, 1=right*/) {
    ...
    if (lr == 0) { dB += PPMCorrectionLeft / 100.0f; dBPPML = dB; }  // left
    if (lr == 1) { dB += PPMCorrectionRight / 100.0f; dBPPMR = dB; } // light
